...
export default function useFilterProducts(products: ProductsData[]) {
    ...
    if (storage.length > 0) {
      filtered = filtered.filter((celular) => {
        return storage.some((value) => {
          return celular.storages.capacity.toString().includes(value);
        });
      });
    }
    ...
    if (color.length > 0) {
      filtered = filtered.filter((item) => {
        return color.some((value) => {
          return item.colors.name.toLowerCase().includes(value);
        });
      });
    }
